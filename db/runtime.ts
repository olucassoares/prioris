...
const globalDatabase = globalThis as typeof globalThis & {
  priorisPostgres?: ReturnType<typeof postgres>;
};
...
function getClient() {
  ...
  globalDatabase.priorisPostgres ??= postgres(databaseUrl, {
    max: 5,
    idle_timeout: 20,
    connect_timeout: 10,
    prepare: false,
  });
  return globalDatabase.priorisPostgres;
}
...
function withReturnedId(query: string) {
  ...
  return table && new Set(["assets", "findings", "audit_events"]).has(table)
    ? `${query} RETURNING id`
    : query;
}
